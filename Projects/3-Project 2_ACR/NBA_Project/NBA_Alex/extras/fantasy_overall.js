...
d3.csv("Resources/nba_players.csv", function (error, nbaData) {
    ...
    nbaData = nbaData.filter(function (x) { return x.GAME_DATE == "2019-04-01" })
    ...
    var yLinearScale = d3.scaleLinear()
        .domain([0, d3.max(nbaData, d => d.NBA_FANTASY_PTS)])
        .range([chartHeight, 0]);
...
positionColor = {
    ...
    circlesGroup
        ...
        .attr("cy", d => yLinearScale(d.NBA_FANTASY_PTS))
        .attr("r", "12")
    ...
    teamGroup
        ...
        .attr("y", d => yLinearScale(d.NBA_FANTASY_PTS) + 3)
        .attr("font-family", "sans-serif")
        .attr("font-size", "8px")
        .attr("fill", "black")
        .attr('text-anchor', 'middle')
    ...
    var toolTip = d3.tip()
        ...
        .offset([80, -60])
        .html(function (d) {
            return (`FANTASY_PTS: ${d.NBA_FANTASY_PTS}<br> ${d.MATCHUP}<br>${d.PLAYER_NAME} `);
        });
