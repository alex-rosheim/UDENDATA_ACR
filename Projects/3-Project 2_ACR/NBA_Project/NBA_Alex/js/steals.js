...
function renderTable(date) {
    ...
    d3.csv("/NBA_Alex/Resources/nba_players.csv", function (error, nbaData) {
        ...
        var yLinearScale = d3.scaleLinear()
            .domain([0, d3.max(nbaData, d => d.STL)])
            .range([chartHeight, 0]);
        ...
        var toolTip = d3.tip()
            ...
            .offset([80, -60])
            .html(function (d) {
                return (`STEALS: ${d.STL}<br> ${d.MATCHUP}<br>${d.PLAYER_NAME}<BR>${date} `);
            });
